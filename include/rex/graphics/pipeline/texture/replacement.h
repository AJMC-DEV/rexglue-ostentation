 ...
 *                  textures/dump/<hash16>_<w>x<h>_<fmt_name>.dds   (default)
 *                  textures/dump/<hash16>_<w>x<h>_<fmt_name>.png   (when texture_dump_format="png")
 *
...
REXCVAR_DECLARE(bool, texture_dump_skip_video_sizes);
...
class TextureReplacement {
 ...
 private:
  ...
  static bool WritePNG_RGBA8(const std::filesystem::path& path,
                             uint32_t width, uint32_t height,
                             const uint8_t* rgba8_rows,
                             uint32_t row_pitch_bytes);
