...
// Forward declarations for embedded Adobe Clean Regular font data.
namespace fonts {
extern const unsigned char kAdobeCleanRegularData[];
extern const int kAdobeCleanRegularSize;
} // namespace fonts

REXCVAR_DEFINE_STRING(ui_theme, "purple", "UI", "ImGui color theme preset")
    .allowed({"red", "orange", "yellow", "green", "blue", "cyan", "magenta",
              "purple", "brown", "light", "dark"})
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

// Applies a full ImGui color palette derived from a named preset.
// All dark themes share the same lightness/saturation ratios; only the hue
// shifts.  "light" inverts the scale to a near-white background.
static void ApplyThemeColors(std::string_view theme) {
  struct Preset {
  ...
  static constexpr Preset kPresets[] = {
      {"red",     0.000f, 1.00f, false},
      {"orange",  0.075f, 1.00f, false},
      {"yellow",  0.145f, 1.00f, false},
      {"green",   0.350f, 1.00f, false},
      {"blue",    0.620f, 1.00f, false},
      {"cyan",    0.500f, 1.00f, false},
      {"magenta", 0.850f, 1.00f, false},
      {"purple",  0.780f, 1.00f, false},
      {"brown",   0.055f, 0.55f, false},
      {"dark",    0.000f, 0.00f, false},
      {"light",   0.620f, 0.50f, true},
  };

  const Preset* preset = &kPresets[7]; // default purple
  for (auto& p : kPresets) {
    if (p.name == theme) { preset = &p; break; }
  }

  auto& col = ImGui::GetStyle().Colors;
  const float h = preset->h, s = preset->s;

  // C(sat_scale, value [,alpha]) → ImVec4 using preset hue.
  auto C = [h, s](float vs, float vv, float va = 1.0f) -> ImVec4 {
    float r, g, b;
    ImGui::ColorConvertHSVtoRGB(h, vs * s, vv, r, g, b);
    return {r, g, b, va};
  };

  if (!preset->light) {
    col[ImGuiCol_Text]                 = ImVec4(0.89f, 0.90f, 0.90f, 1.00f);
    col[ImGuiCol_TextDisabled]         = ImVec4(0.60f, 0.60f, 0.60f, 1.00f);
    col[ImGuiCol_WindowBg]             = C(0.80f, 0.06f);
    col[ImGuiCol_ChildBg]              = ImVec4(0.00f, 0.00f, 0.00f, 0.00f);
    col[ImGuiCol_PopupBg]              = C(0.70f, 0.12f, 0.99f);
    col[ImGuiCol_Border]               = C(1.00f, 0.35f);
    col[ImGuiCol_BorderShadow]         = ImVec4(0.00f, 0.00f, 0.00f, 0.00f);
    col[ImGuiCol_FrameBg]              = ImVec4(0.80f, 0.80f, 0.80f, 0.30f);
    col[ImGuiCol_FrameBgHovered]       = ImVec4(0.90f, 0.80f, 0.80f, 0.40f);
    col[ImGuiCol_FrameBgActive]        = ImVec4(0.90f, 0.65f, 0.65f, 0.45f);
    col[ImGuiCol_TitleBg]              = C(1.00f, 0.40f);
    col[ImGuiCol_TitleBgActive]        = C(1.00f, 0.65f);
    col[ImGuiCol_TitleBgCollapsed]     = C(1.00f, 0.33f);
    col[ImGuiCol_MenuBarBg]            = C(1.00f, 0.35f);
    col[ImGuiCol_ScrollbarBg]          = C(1.00f, 0.40f, 0.59f);
    col[ImGuiCol_ScrollbarGrab]        = C(1.00f, 0.68f, 0.68f);
    col[ImGuiCol_ScrollbarGrabHovered] = C(1.00f, 1.00f, 0.62f);
    col[ImGuiCol_ScrollbarGrabActive]  = C(1.00f, 0.91f, 0.40f);
    col[ImGuiCol_CheckMark]            = C(0.20f, 0.90f, 0.50f);
    col[ImGuiCol_SliderGrab]           = ImVec4(1.00f, 1.00f, 1.00f, 0.30f);
    col[ImGuiCol_SliderGrabActive]     = C(0.85f, 0.75f);
    col[ImGuiCol_Button]               = C(0.80f, 0.56f, 0.60f);
    col[ImGuiCol_ButtonHovered]        = C(0.87f, 0.72f);
    col[ImGuiCol_ButtonActive]         = C(0.85f, 0.60f);
    col[ImGuiCol_Header]               = C(1.00f, 0.40f, 0.71f);
    col[ImGuiCol_HeaderHovered]        = C(1.00f, 0.60f, 0.80f);
    col[ImGuiCol_HeaderActive]         = C(1.00f, 0.75f, 0.80f);
    col[ImGuiCol_Separator]            = C(1.00f, 0.35f);
    col[ImGuiCol_SeparatorHovered]     = C(0.55f, 0.89f);
    col[ImGuiCol_SeparatorActive]      = C(0.80f, 0.50f);
    col[ImGuiCol_ResizeGrip]           = ImVec4(1.00f, 1.00f, 1.00f, 0.30f);
    col[ImGuiCol_ResizeGripHovered]    = ImVec4(1.00f, 1.00f, 1.00f, 0.60f);
    col[ImGuiCol_ResizeGripActive]     = ImVec4(1.00f, 1.00f, 1.00f, 0.90f);
    col[ImGuiCol_Tab]                  = C(0.80f, 0.56f, 0.60f);
    col[ImGuiCol_TabHovered]           = C(0.87f, 0.72f);
    col[ImGuiCol_TabActive]            = C(0.85f, 0.60f);
    col[ImGuiCol_TabUnfocused]         = ImVec4(0.80f, 0.80f, 0.80f, 0.30f);
    col[ImGuiCol_TabUnfocusedActive]   = ImVec4(0.90f, 0.80f, 0.80f, 0.40f);
    col[ImGuiCol_PlotLines]            = ImVec4(1.00f, 1.00f, 1.00f, 1.00f);
    col[ImGuiCol_PlotLinesHovered]     = ImVec4(0.90f, 0.70f, 0.00f, 1.00f);
    ...
    col[ImGuiCol_TextSelectedBg]       = C(1.00f, 1.00f, 0.21f);
    col[ImGuiCol_ModalWindowDimBg]     = ImVec4(0.20f, 0.20f, 0.20f, 0.35f);
  } else {
    // Light theme: near-white background, tinted with preset hue.
    col[ImGuiCol_Text]                 = ImVec4(0.00f, 0.00f, 0.00f, 1.00f);
    col[ImGuiCol_TextDisabled]         = ImVec4(0.35f, 0.35f, 0.38f, 1.00f);
    col[ImGuiCol_WindowBg]             = C(0.04f, 0.97f);
    col[ImGuiCol_ChildBg]              = ImVec4(0.00f, 0.00f, 0.00f, 0.00f);
    col[ImGuiCol_PopupBg]              = C(0.03f, 0.99f);
    col[ImGuiCol_Border]               = C(0.30f, 0.72f);
    col[ImGuiCol_BorderShadow]         = ImVec4(0.00f, 0.00f, 0.00f, 0.00f);
    col[ImGuiCol_FrameBg]              = C(0.10f, 0.90f);
    col[ImGuiCol_FrameBgHovered]       = C(0.15f, 0.85f);
    col[ImGuiCol_FrameBgActive]        = C(0.20f, 0.82f);
    col[ImGuiCol_TitleBg]              = C(0.25f, 0.85f);
    col[ImGuiCol_TitleBgActive]        = C(0.40f, 0.75f);
    col[ImGuiCol_TitleBgCollapsed]     = C(0.15f, 0.90f);
    col[ImGuiCol_MenuBarBg]            = C(0.15f, 0.90f);
    col[ImGuiCol_ScrollbarBg]          = C(0.08f, 0.94f);
    col[ImGuiCol_ScrollbarGrab]        = C(0.30f, 0.72f);
    col[ImGuiCol_ScrollbarGrabHovered] = C(0.40f, 0.62f);
    col[ImGuiCol_ScrollbarGrabActive]  = C(0.50f, 0.52f);
    col[ImGuiCol_CheckMark]            = C(0.80f, 0.40f);
    col[ImGuiCol_SliderGrab]           = C(0.40f, 0.68f);
    col[ImGuiCol_SliderGrabActive]     = C(0.50f, 0.55f);
    col[ImGuiCol_Button]               = C(0.22f, 0.82f);
    col[ImGuiCol_ButtonHovered]        = C(0.32f, 0.74f);
    col[ImGuiCol_ButtonActive]         = C(0.42f, 0.66f);
    col[ImGuiCol_Header]               = C(0.22f, 0.84f, 0.80f);
    col[ImGuiCol_HeaderHovered]        = C(0.32f, 0.76f, 0.80f);
    col[ImGuiCol_HeaderActive]         = C(0.42f, 0.68f, 0.80f);
    col[ImGuiCol_Separator]            = C(0.28f, 0.74f);
    col[ImGuiCol_SeparatorHovered]     = C(0.40f, 0.62f);
    col[ImGuiCol_SeparatorActive]      = C(0.50f, 0.55f);
    col[ImGuiCol_ResizeGrip]           = C(0.20f, 0.78f, 0.30f);
    col[ImGuiCol_ResizeGripHovered]    = C(0.30f, 0.68f, 0.60f);
    col[ImGuiCol_ResizeGripActive]     = C(0.40f, 0.58f, 0.90f);
    col[ImGuiCol_Tab]                  = C(0.20f, 0.86f);
    col[ImGuiCol_TabHovered]           = C(0.32f, 0.76f);
    col[ImGuiCol_TabActive]            = C(0.28f, 0.78f);
    col[ImGuiCol_TabUnfocused]         = C(0.10f, 0.92f);
    col[ImGuiCol_TabUnfocusedActive]   = C(0.18f, 0.87f);
    col[ImGuiCol_PlotLines]            = ImVec4(0.40f, 0.40f, 0.40f, 1.00f);
    col[ImGuiCol_PlotLinesHovered]     = C(0.70f, 0.45f);
    col[ImGuiCol_PlotHistogram]        = C(0.60f, 0.55f);
    col[ImGuiCol_PlotHistogramHovered] = C(0.80f, 0.42f);
    col[ImGuiCol_TextSelectedBg]       = C(0.40f, 0.72f, 0.35f);
    col[ImGuiCol_ModalWindowDimBg]     = ImVec4(0.20f, 0.20f, 0.20f, 0.35f);
  }
}

// Kept for reference — previously used ProggyTiny. Remove when confident.
...
void ImGuiDrawer::Initialize() {
  ...
  // Setup the font glyphs — Adobe Clean Regular embedded at build time.
  ...
  io.Fonts->AddFontFromMemoryTTF(
      ...
      const_cast<unsigned char*>(fonts::kAdobeCleanRegularData),
      fonts::kAdobeCleanRegularSize, 14.0f, &font_config);
  ...
  // Spectrum layout: rounded corners + thin border (keeps the Adobe feel)
  ...
  // Apply color theme from cvar (defaults to "purple").
  ApplyThemeColors(REXCVAR_GET(ui_theme));
...
void ImGuiDrawer::Draw(UIDrawContext& ui_draw_context) {
  ...
  // Re-apply theme colors if the cvar changed since last frame.
  {
    static std::string last_theme;
    const std::string& cur = REXCVAR_GET(ui_theme);
    if (cur != last_theme) {
      last_theme = cur;
      ApplyThemeColors(cur);
    }
  }
